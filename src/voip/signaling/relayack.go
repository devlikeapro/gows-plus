...
func ParseRelayFromAck(ackNode *waBinary.Node) ParsedRelayAck {
	...
	for _, child := range wanode.NodeChildren(ackNode) {
		...
		res.Relays = append(res.Relays, parseRelayTe2Endpoints(relayKey, relayContent)...)
	}
...
// parseRelayTe2Endpoints parses modern WhatsApp relay blocks (token/auth_token/te2).
func parseRelayTe2Endpoints(relayKey string, relayContent []waBinary.Node) []core.RelayEndpoint {
